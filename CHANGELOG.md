...
- **An assembly names every preprocessed sample it was built from.** One sample per assembly says everything about an individual assembly and nothing about a coassembly; Airtable keeps it the other way round, one row per sample carrying the assembly code it belongs to, which is how a binning run's sample sheet is written. ehi-core holds these as `assembly_samples`, which also carry each sample's mapping rate, Nonpareil completeness and BAM, and the `entries` route unfolds them back to Airtable's shape so the sheet comes out the same whichever database it was read from. `ehio binning --input` tells the core the grouping it read from Airtable.
...
- **A finished DMB batch can have its genomes functionally annotated again under the current drakkar**, without being dereplicated or profiled again. Setting a batch to the new `SCANNING_REANNOTATE_STATUS` (default `Reannotate`) launches it like any other pending batch; only DMB batches are scanned for the status, since they are the one module whose results hold a genome catalogue that can be put back on disk and annotated on its own. Until now `ehio annotating` had no launch path of its own — it existed only as the tail of a `quantifying` batch — so re-annotating an old batch meant going through `Resume`, which calls `drakkar profiling` again and needs the reads and the PPR links the annotation itself has no use for.
  - **`ehio annotating --stage`** puts the batch's genomes back on the cluster. Airtable records how many MAGs came out of dereplication but not which ones, so the catalogue is read from the batch's own counts table on ERDA — `DMB/{batch}/{batch}_counts.tsv.gz`, one row per dereplicated genome — and each of those genomes is downloaded from the FASTA URL on its `MAG_ENTRY` record, decompressed, and written as `{mag}.fa` into `profiling_genomes/drep/dereplicated_genomes`, which is where a profiling run would have left it. A genome already staged is kept, so a re-annotation that stopped halfway downloads nothing twice, and `--redownload` forces a fresh copy. A genome with no MAG record, no FASTA URL, or a URL that cannot be downloaded stops the batch before drakkar starts, with every problem reported at once instead of one per run. `--genomes-file` supplies the catalogue by hand for a batch whose counts table is missing or oddly shaped.
  - The launch script of a re-annotation is the functional annotation and nothing else: stage, gene annotation over the staged catalogue, and — for an `all` batch — cluster annotation. `drakkar profiling` and `ehio quantifying --output` are not called at all, so the counts, the mapping rates and the dereplicated MAG count already in Airtable are left exactly as they are.
  - **Taxonomy is not re-run**, and the taxonomy already recorded is not touched. A genome's classification is a property of the genome, fixed when it was binned; dereplicating it neither changes it nor produces a better one, so re-running GTDB-Tk over a DMB catalogue would rewrite the ranks of every MAG in it against whichever GTDB release happened to be installed — a different operation from annotating them again. `--reannotate` therefore makes `ehio annotating --output` skip the taxonomy side entirely: the ranks on the `MAG_ENTRY` records, and `{batch}_genome_taxonomy.tsv.gz` and the trees in `DMB/{batch}` on ERDA, are left as they are even when the output directory holds a `genome_taxonomy.tsv` from some earlier run.
  - `ehio annotating --input` runs with `--rerun` on this path. After a finished batch every MAG already carries the batch's annotation level, so without it the input step would write an empty file and the `[ -s ... ]` guard would skip the whole annotation — the batch would go to `Done` having run nothing. That also leaves the cluster-upgrade file empty, so the single `function` run covers the whole catalogue.
  - The drakkar version already on the batch record is kept and the new one appended to it (`2.4.4/2.5.0`) rather than overwriting it. A re-annotation runs in an output directory holding the annotation runs and nothing else, so the version read from it knows nothing about the drakkar that dereplicated and profiled the batch however long ago; that version survives only in Airtable. `--rerun` alongside `--reannotate` replaces `ANN/{batch}` on ERDA, whose per-genome tables the new ones supersede.
  - A re-annotation clears a stale Snakemake lock first, like a resumed batch, so a run that was killed can simply be set back to `Reannotate`.
...
- `ehio annotating --output` never wrote a single functional annotation metric to `MAG_ENTRY`. `coding_density`, `genes_number`, `genes_unannotated`, `genes_kegg` and `annotated` have been empty on every MAG since the module was added. drakkar names the per-genome gene table after the genome FASTA with its suffix stripped and `_genes` appended — `EHA00123_bin_1.fa` gives `final/EHA00123_bin_1_genes.tsv` — and ehio keyed the parsed table by the file stem plus `.fa`, `EHA00123_bin_1_genes.fa`, which is the name of no MAG record. The gene table and the MAG record are now matched on the drakkar MAG id, the FASTA name with its suffix stripped, which is how the taxonomy table has always been keyed. Only `*_genes.tsv` is read: the `*_clusters.tsv` written beside it is a different table with a different schema and was being parsed as a gene table too.
  - `MAG_ENTRY_ANNOTATED` was part of that: it is set as the metrics are written, and it is what `ehio annotating --input` reads to decide which genomes still need annotating. Empty for every MAG, no genome ever matched the batch's annotation level, so every DMB batch sent every one of its genomes through the full annotation again, however often they had been annotated before.
- The per-genome gene table is parsed as the long-form table drakkar has written since 2.0.0. That release replaced the wide table — one row per gene, one column per database — with a lossless evidence table holding one row per accepted hit, so a gene appears once per source, once more for every additional ranked hit within a source, and always at least once as a `prodigal` row carrying the gene call itself. The `kegg`, `ec`, `pfam` and `cazy` columns ehio read are gone. Read as if it were still the wide table, a MAG with 3 genes and 7 hit rows reported 7 genes, 0 of them with KEGG, all 7 unannotated, and a coding density of 2.52 — a figure that cannot exceed 1. Every metric is now counted over distinct genes: `genes_number` is the genes drakkar predicted, `genes_kegg` those with a KEGG hit, `genes_unannotated` those with no KEGG, Pfam or CAZy hit, and coding density sums each gene once, using the explicit `contig` column and drakkar's 1-based inclusive coordinates. The 1.x wide table is still read, recognised by having no `source` column.
- `cluster_annotations.tsv.xz` is transferred to `Data/DMB/{batch}/` as `{batch}_cluster_annotations.tsv.xz`, alongside the gene table that was already going there. `ehio` asks drakkar for cluster annotation — that is what the `all` annotation type and the `genes` → `all` upgrade path are for — and then left the dbCAN gene clusters, antiSMASH regions, geNomad mobile elements and defense systems behind in the output directory.
- ehio reads the output layout drakkar 2.5.0 introduced. That release collected everything a run records about itself — the run metadata (`drakkar_<run id>.yaml`), the failure table, the Snakemake log and the benchmark roll-up — into a `logging/` subdirectory of the output directory, and renamed the failure table from `drakkar_<run id>_failures.tsv` to `drakkar_<run id>.failures.tsv`. Two things in ehio looked for those files in the output root and quietly stopped finding them:
  - The guard that catches a drakkar call exiting 0 without running anything — a Snakemake lock left by a killed batch, a missing input file — reads the run metadata and checks it is stamped `status: success`. With no metadata in the output root it took the directory for one written by a drakkar too old to record runs and skipped the check altogether, which is exactly the silent-failure path it exists to close. It now searches `logging/` and the output root, so it keeps working under both layouts and for a batch launched under one and resumed under the other. The run id is matched by shape (`drakkar_<8 digits>-<6 digits>.yaml`) rather than by a `drakkar_*.yaml` glob, which also stops the benchmark roll-up an older drakkar wrote beside the metadata as `drakkar_<run id>_resources.yaml` from being read as a run.
  - The failure table attached to the batch record on error is found again. A failed batch under drakkar 2.5.0 was still flagged as failed in Airtable, but with no table to say which jobs died and why. Both directories and both file names are searched, and the newest table across them wins.

### Changed

- The drakkar version written to a finished batch (`EHI_PPR_BATCH_DRAKKAR_VERSION` and its binning, quantifying and AMR counterparts) is read from the run metadata in the output directory instead of from `drakkar --version`. The field now says which version actually produced the results rather than which one happens to be installed when the batch is wrapped up — the two differ whenever drakkar was updated while the batch was queued or running, which is how a batch finished by 2.4.5 was recorded as processed by 2.5.0.
- A batch processed by more than one drakkar version reports all of them, oldest run first and separated by a slash: `2.4.4/2.4.5`. One batch holds one metadata file per drakkar run — quantifying alone calls drakkar four times, and a resumed batch adds a run for every launch — so an upgrade midway through a batch leaves part of the results built by each version. A failed run counts too, since the run that resumes it builds on what it left behind, while a run of a command that executes no workflow (`drakkar config` wrote run metadata as well in older versions) does not.
- `ehio annotating --output` writes the drakkar version of a DMB batch again as it marks the batch done. It is the last step of the batch, and the taxonomy, function and cluster annotation runs all happen after `ehio quantifying --output` has written the version of the profiling run — so without this, the one module that calls drakkar four times could never report an upgrade that happened between the first call and the last.
- Both layouts are read, so a batch that ran under drakkar 2.4.x still reports its version. When the output directory holds no run metadata at all — it was cleaned up, or the drakkar that ran predates the metadata — the installed drakkar is asked as before. The version is now read at the start of `ehio <module> --output`, while the output directory is certain to still be there, rather than after the transfer that may delete it.

### Notes

- Two drakkar changes in the same range need nothing from ehio, but change what a finished batch looks like. `MIN_BINNING_ASSEMBLY_MB` (drakkar 2.5.0, default 10 MB) is now applied by all four binners, so an assembly below it yields no bins at all: the entry comes back with `bins_number` 0 and no MAG records, where `metabat2` alone used to produce some. And the drakkar 2.4.4 fix that stops MetaBAT2 handing its unbinned contigs to Binette corrects the `metabat2_bins` column of `cataloging.tsv`, which ehio does not read — the `final_bins` column it does read is unaffected.
## [0.7.0] - 2026-08-31
